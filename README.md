...
- `/api/dataset/stats` API를 추가해 데이터셋 요약 통계를 조회할 수 있게 했습니다.
- 저장된 캐릭터 스냅샷, 장비, 장신구 효과, 팔찌 효과, 스톤, 기억 입력 행 수를 집계합니다.
- 최근 저장된 캐릭터 스냅샷 목록, 직업별 저장 수, 스톤 타입 분포, 장신구 공식 매칭 성공/실패 개수를 제공합니다.
- 프론트에 데이터셋 상태 카드를 추가해 앱 안에서 누적 데이터 상태를 바로 확인할 수 있게 했습니다.
- 기존 `/api/dataset/status`는 파일 수/행 수/용량/경로 확인용으로 유지합니다.
