...
- 팔찌 고정 옵션 수와 랜덤 부여효과 슬롯 수를 기본 자동 추정하도록 수정했습니다.
- 자동 추정 규칙은 `총 옵션 3개 → 고정 1 / 랜덤 2`, `총 옵션 4개 → 기본 고정 2 / 랜덤 2`, `총 옵션 5개 → 고정 2 / 랜덤 3`입니다.
- 단, 총 옵션 4개여도 특수옵션이 3개 감지되면 `고정 1 / 랜덤 3`으로 우선 추정합니다.
- 사용자가 고정 옵션 수나 랜덤 슬롯 수를 직접 선택하면 수동 입력값이 자동 추정보다 우선합니다.
- 리포트 카드에 계산 기준과 추정 사유를 표시합니다.
- 현재 팔찌 효과 전체를 하나의 랜덤 목표로 계산하지 않고, 랜덤 부여효과 슬롯 기준 기대값만 계산합니다.
...
- 기억 기반 보조 판정을 브라우저 localStorage에 캐릭터명/서버명 기준으로 저장하고 불러옵니다.
