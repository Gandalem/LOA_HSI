...
- 팔찌 T4 공식 확률표의 등급별 효과 개수 확률, 카테고리 확률, 중복 제외 규칙을 현재 팔찌 효과와 연결했습니다.
- `expectedValues.officialBraceletT4`에 현재 팔찌 효과별 공식 카테고리 매칭 결과를 추가했습니다.
- 팔찌 효과를 기본 효과, 전투 특성, 특수 효과 카테고리로 분류하고 역할별 핵심/보조/조건부 여부를 표시합니다.
- 공식으로 분리된 효과 개수 확률과 카테고리 확률을 이용해 카테고리 기준 조합 확률과 기대 시도 수를 제공합니다.
- 옵션 개별 수치 구간별 표기확률은 아직 공식 JSON에 완전 분리하지 않았으므로, v53에서는 카테고리 기준 확률로 과장 없이 표시합니다.
...
- 팔찌 T4: 공식 효과 개수/카테고리 확률과 현재 팔찌 효과 카테고리를 매칭합니다. 옵션 개별 수치 구간별 표기확률은 후속 버전에서 분리 예정입니다.
...
- `officialBraceletT4` 결과를 프론트 리포트 UI에 별도 카드로 표시합니다.
